...
public class JOGL2Nehe12DisplayList implements GLEventListener, KeyListener {
	private static String TITLE = "NeHe Lesson #12: Display Lists"; // window's
																	// title
	...
	private static float rotateAngleX = 0f; // rotational angle for x-axis in
											// degree
	private static float rotateAngleY = 0f; // rotational angle for y-axis in
											// degree
	...
	public void display(GLAutoDrawable drawable) {
		...
		for (int y = 1; y < 6; y++) { // loop through the y-axis (5 rows)
			for (int x = 0; x < y; x++) { // loop through the x-axis (y columns)
				...
				gl.glTranslatef(1.4f + x * 2.8f - y * 1.4f,
						(6.0f - y) * 2.4f - 7.0f, -20.0f);
				gl.glRotatef(45.0f - (2.0f * y) + rotateAngleX, 1.0f, 0.0f,
						0.0f);
