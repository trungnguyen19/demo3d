...
public class Lesson37 implements KeyListener {
	...
	Texture myShaderTexture;

	int shaderTexture[] = new int[1], // Storage For One Texture ( NEW )
			screenWidth, screenHeight, canvasHeight, canvasWidth,
			xLocation,
			yLocation, polyNum = 0; // Number Of Polygons ( NEW )
	...
	// Math Functions
	float DotProduct(VECTOR V1, VECTOR V2) { // Calculate The Angle Between The
												// 2 Vectors ( NEW )
	...
	float Magnitude(VECTOR V) { // Calculate The Length Of The Vector ( NEW )
		return (float) Math.sqrt(V.X * V.X + V.Y * V.Y + V.Z * V.Z); // Return
																		// The
																		// Length
																		// Of
																		// The
	...
	void RotateVector(MATRIX M, VECTOR V, VECTOR D) { // Rotate A Vector Using
														// The Supplied Matrix (
														// NEW )

		D.X = (M.Data[0] * V.X) + (M.Data[4] * V.Y) + (M.Data[8] * V.Z); // Rotate
																			// Around
																			// The
																			// X
																			// Axis
																			...
																			// NEW
																			// )
		D.Y = (M.Data[1] * V.X) + (M.Data[5] * V.Y) + (M.Data[9] * V.Z); // Rotate
																			// Around
																			// The
																			// Y
																			// Axis
																			...
																			// NEW
																			// )
		D.Z = (M.Data[2] * V.X) + (M.Data[6] * V.Y) + (M.Data[10] * V.Z); // Rotate
																			// Around
																			// The
																			// Z
																			// Axis
	...
	Lesson37() {
		...
		default:
			...
			frame.setIconImage(new ImageIcon(
					"src/main/java/nehe/lesson37/data/icon.png").getImage());
		}
	...
	public class initRenderer implements GLEventListener {
		public void init(GLAutoDrawable drawable) {
			...
			info;
			float shaderData[] = new float[96]; // Storate For The 96 Shader
												// Values ( NEW )

			// Start Of User Initialization
			gl.glHint(GL.GL_PERSPECTIVE_CORRECTION_HINT, GL.GL_NICEST); // Realy
																		// Nice
																		// perspective
																		// calculations
			...
			try {
				shader = new FileReader(
						"src/main/java/nehe/lesson37/data/shader.txt"); // Open
																		// The
																		// Shader
				// File ( NEW )
			...
			for (i = 0; i < 96; i += 3)
				...
				shaderData[i] = shaderData[i + 1] = shaderData[i + 2] = Float
						.parseFloat(tokenizer.nextToken()); // Copy Over The
															// Value ( NEW )

			gl.glGenTextures(1, shaderTexture, 0); // Get A Free Texture ID (
													// NEW )
			gl.glBindTexture(GL.GL_TEXTURE_1D, shaderTexture[0]); // Bind This
																	// Texture.
																	// From Now
																	// On It
																	// Will Be
			...
			ByteBuffer temp = ByteBuffer.allocateDirect(shaderData.length
					* Float.SIZE); // 4
			temp.order(ByteOrder.nativeOrder());
			gl.glTexImage1D(GL.GL_TEXTURE_1D, 0, GL.GL_RGB, 32, 0, GL.GL_RGB,
					GL.GL_FLOAT,
					(FloatBuffer) temp.asFloatBuffer().put(shaderData).flip()); // Upload
																				// (
																				// NEW
																				// )
		...
		public void display(GLAutoDrawable drawable) {
			...
			if (outlineSmooth) { // Check To See If We Want Anti-Aliased Lines (
									// NEW )
				gl.glHint(GL.GL_LINE_SMOOTH_HINT, GL.GL_NICEST); // Use The Good
																	// Calculations
																	// ( NEW )
			...
			gl.glTranslatef(0.0f, 0.0f, -2.0f); // Move 2 Units Away From The
												// Screen ( NEW )
			gl.glRotatef(modelAngle, 0.0f, 1.0f, 0.0f); // Rotate The Model On
														// It's Y-Axis ( NEW )

			FloatBuffer temp = FloatBuffer.allocate(TmpMatrix.Data.length);
			gl.glGetFloatv(GL.GL_MODELVIEW_MATRIX, temp); // Get The
																	// Generated
																	// Matrix (
																	// NEW )
			for (int z = 0;z < temp.capacity();z++){
				TmpMatrix.Data[z] = temp.get(z);
			}
			
			// Cel-Shading Code //
			gl.glEnable(GL.GL_TEXTURE_1D); // Enable 1D Texturing ( NEW )
			gl.glBindTexture(GL.GL_TEXTURE_1D, shaderTexture[0]); // Bind Our
																	// Texture (
																	// NEW )
			gl.glColor3f(1.0f, 1.0f, 1.0f); // Set The Color Of The Model ( NEW
											// )

			gl.glBegin(GL.GL_TRIANGLES); // Tell OpenGL That We're Drawing
											// Triangles
			...
			for (i = 0; i < polyNum; i++)
				...
				for (j = 0; j < 3; j++) { // Loop Through Each Vertex ( NEW )
					...
					TmpNormal.X = polyData[i].Verts[j].Nor.X; // Fill Up The
																// TmpNormal
																// Structure
																// With
					TmpNormal.Y = polyData[i].Verts[j].Nor.Y; // The Current
																// Vertices'
																// Normal Values
																// ( NEW )
					...
					TmpShade = DotProduct(TmpVector, lightAngle); // Calculate
																	// The Shade
																	// Value (
																	// NEW )
					if (TmpShade < 0.0f)
					...
					gl.glTexCoord1f(TmpShade); // Set The Texture Co-ordinate As
												// The Shade Value ( NEW )
					gl.glVertex3f(polyData[i].Verts[j].Pos.X,
							polyData[i].Verts[j].Pos.Y,
							polyData[i].Verts[j].Pos.Z); // Send The Vertex
															// Position ( NEW )
				}
			...
			if (outlineDraw) { // Check To See If We Want To Draw The Outline (
				...
				gl.glBlendFunc(GL.GL_SRC_ALPHA, GL.GL_ONE_MINUS_SRC_ALPHA);// Set
																			// The
																			// Blend
																			// Mode
																			// (
				...
				gl.glCullFace(GL.GL_FRONT); // Don't Draw Any Front-Facing
											// Polygons ( NEW )

				gl.glDepthFunc(GL.GL_LEQUAL); // Change The Depth Mode ( NEW )

				ByteBuffer temp2 = ByteBuffer
						.allocateDirect(outlineColor.length*Float.SIZE);
				temp2.order(ByteOrder.nativeOrder());
				gl.glColor3fv((FloatBuffer) temp2.asFloatBuffer()
						.put(outlineColor).flip()); // Set The Outline Color (
													// NEW )
	...
	void ReadMesh() {
		...
		URL fileSource = null;
		try {
			fileSource = new URL("file", "localhost",
					"src/main/java/nehe/lesson37/data/model.txt");
