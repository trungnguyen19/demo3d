...
public class MySupport {
	...
	public static void main(String[] args) {
		String outFile = null;
		if (args.length < 1) {
			outFile = MyConstants.dataPath + "videos/output.mp4";
		...
		try {
			MySupport videoEncoder = new MySupport(outFile);
			int index = 0;
			while (index < 15 * videoEncoder.frameRate.getDouble()) {
				...
				try {
					...
					Thread.sleep((long) (1000 / videoEncoder.frameRate
							.getDouble()));
	 ...
	 *            File to write to.
	 */
	public MySupport(String outFile) {
		try {
			robot = new Robot();
	...
	public void encodeImage(BufferedImage originalImage) {
		BufferedImage worksWithXugglerBufferedImage = convertToType(
