...
public class Lesson42 {
	...
	static class Renderer implements GLEventListener, KeyListener {
		...
		void updateTex(int dmx, int dmy)					/* Update Pixel dmx, dmy On The Texture */
		{
			...
			tex_data.put(0+((dmx+(width*dmy))*3),Byte.MAX_VALUE);	/* Set Red Pixel To Full Bright */
			tex_data.put(1+((dmx+(width*dmy))*3),Byte.MAX_VALUE);	/* Set Green Pixel To Full Bright */
			tex_data.put(2+((dmx+(width*dmy))*3),Byte.MAX_VALUE);	/* Set Blue Pixel To Full Bright */
		}
		...
		void update (float milliseconds)
		{
			...
			for (x=0; x<width; x+=2)			  /* Loop Through All The Rooms */
			{
				...
				{
					if (tex_data.get(((x+(width*y))*3))==0)	/* If Current Texture Pixel (Room) Is Blank */
			...
			if (done)														/* If done Is True Then There Were No Unvisited Rooms */
			{
				...
				System.out.println("Lesson 42: Multiple Viewports... 2003 NeHe Productions... Maze Complete!");
				try {
					Thread.sleep(5000);
				...
				System.out.println("Lesson 42: Multiple Viewports... 2003 NeHe Productions... Building Maze!");
				reset();
			}

			/* Check To Make Sure We Are Not Trapped (Nowhere Else To Move) */
			if (((tex_data.get((((mx+2)+(width*my))*3))==255) || mx>(width-4)) && ((tex_data.get((((mx-2)+(width*my))*3))==255) || mx<2) &&
				((tex_data.get(((mx+(width*(my+2)))*3))==255) || my>(height-4)) && ((tex_data.get(((mx+(width*(my-2)))*3))==255) || my<2))
			{
				do																/* If We Are Trapped */
				{
					mx=(int)(Math.random()%width);			/* Pick A New Random X Position */
					my=(int)(Math.random()%height);		/* Pick A New Random Y Position */
				}
				while (tex_data.get(((mx+(width*my))*3))==0);	/* Keep Picking A Random Position Until We Find */
			...
			{
				if (tex_data.get((((mx+2)+(width*my))*3))==0)	/* And If The Room To The Right Has Not Already Been Visited */
				{
			...
			{
				if (tex_data.get(((mx+(width*(my+2)))*3))==0)	/* And If The Room Below Has Not Already Been Visited */
				{
			...
			{
				if (tex_data.get((((mx-2)+(width*my))*3))==0)	/* And If The Room To The Left Has Not Already Been Visited */
				{
					updateTex(mx-1,my);							/* Update The Texture To Show Path Cut Out Between Rooms */
			...
			{
				if (tex_data.get(((mx+(width*(my-2)))*3))==0)	/* And If The Room Above Has Not Already Been Visited */
				{
					updateTex(mx,my-1);							/* Update The Texture To Show Path Cut Out Between Rooms */
...
//			System.out.println(Math.random());
			...
			gl.glTexSubImage2D(GL.GL_TEXTURE_2D, 0, 0, 0, width, height, GL.GL_RGB, GL.GL_UNSIGNED_BYTE, tex_data);
			...
			for (loop=0; loop<4; loop++)				/* Loop To Draw Our 4 Views */
			{
				...
				if (loop==1)	/* If We Are Drawing The Second Scene */
				{
					...
					glu.gluPerspective( 45.0, (float)(width)/(float)(height), 0.1f, 500.0 ); 
				}
				...
				if (loop==0)	/* Are We Drawing The First Image?  (Original Texture... Ortho) */
				{
					...
					gl.glTexCoord2f(1.0f, 0.0f); gl.glVertex2i(window_width/2, 0              );
					gl.glTexCoord2f(0.0f, 0.0f); gl.glVertex2i(0,              0              );
					gl.glTexCoord2f(0.0f, 1.0f); gl.glVertex2i(0,              window_height/2);
					gl.glTexCoord2f(1.0f, 1.0f); gl.glVertex2i(window_width/2, window_height/2);
		...
		void reset() {
			...
			{
				r[loop]=(byte)(Math.random()%256);									/* Pick A Random Red Color (Bright) */
				g[loop]=(byte)(Math.random()%256);									/* Pick A Random Green Color (Bright) */
				b[loop]=(byte)(Math.random()%256);									/* Pick A Random Blue Color (Bright) */
			}
...
//			}
			...
			tex_data=ByteBuffer.allocateDirect(width*height*3);		/* Allocate Space For Our Texture */
			...
			gl.glTexImage2D(GL.GL_TEXTURE_2D, 0, GL.GL_RGB, width, height, 0, GL.GL_RGB, GL.GL_UNSIGNED_BYTE, tex_data);
