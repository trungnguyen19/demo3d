package nehe.lesson17.Texture_Font;
...
import nehe.lesson17.LoadImageTypes.LoadBMPImages;
import nehe.lesson17.LoadImageTypes.LoadPNGImages;
...
public class Lesson17 extends Frame implements GLEventListener, KeyListener {
	...
	public Lesson17(Dimension dim, boolean fscreen) {
		...
		glCanvas = new GLCanvas();// GLDrawableFactory.getFactory().createGLCanvas(glCapabilities);
	...
	public void display(GLAutoDrawable gLDrawable) {
		...
		glPrint(((int) ((480 + (screensize.width - 500) * Math.cos(cnt1)))),
				...
				"NeHe", 0, gl); // Print GL Text To The Screen
		...
		glPrint(((int) ((480 + (screensize.width - 500) * Math.cos(cnt2)))),
				...
				"OpenGL", 1, gl); // Print GL Text To The Screen

		gl.glColor3f(0.0f, 0.0f, 1.0f); // Set Color To Blue
		glPrint(((int) (380 + (screensize.width - 500)
				* Math.cos((cnt2 + cnt1) / 5))), 100,
				"Giuseppe D'Agata/Nick Campbell", 0, gl); // Draw Text To The
															// Screen

		gl.glColor3f(1.0f, 1.0f, 1.0f); // Set Color To White
		glPrint(((int) (382 + (screensize.width - 500)
				* Math.cos((cnt2 + cnt1) / 5))), 100,
				"Giuseppe D'Agata/Nick Campbell", 0, gl); // Draw Offset Text To
															// The Screen
		if (!(fullscreen)) {
			cnt1 += 0.01f;
			cnt2 += 0.0081f;
	...
	public void init(GLAutoDrawable gLDrawable) {
		...
		String resourceName = "nehe/lesson17/Texture_Font/data/Font.bmp";
		String resourceName2 = "nehe/lesson17/Texture_Font/data/Bumps.bmp";
		...
		// I don't know why, as of now, the PNG images come out crappy and
		...
		gl.glBlendFunc(GL.GL_SRC_ALPHA, GL.GL_ONE); // Really Nice Perspective
													// Calculations
	...
	/**
	 * Called by init(). This method is used to create our font as a display
	 * list. This list gets stored in the int base and is then created. This is
	 * not just one list btw, it is 256 lists for 256 characters. We create a
	 * list for each character.
	 * 
	 * @param gl
	 *            send the gl object to this method for use.
	 */
	public void generateFont(GL gl) {
		...
		for (loop = 0; loop < 256; loop++) {
			cx = ((float) (loop % 16)) / 16.0f; // X Position Of Current
												// Character
			cy = ((float) (loop / 16)) / 16.0f; // Y Position Of Current
												// Character
	...
	/**
	 * This method is called when the canvas is destroyed...When the user
	 * presses esc. This method deletes the font list we created.
	 * 
	 * @param gl
	 *            send the gl object to this method for use.
	 */
	 ...
	 * 
	 * @param x
	 *            where to place the string on the x axis
	 * @param y
	 *            where to place the string on the y axis
	 * @param text
	 *            the string to display
	 * @param set
	 *            are we using the first character set or the second...
	 * @param gl
	 *            send the gl object to this method for use.
	 */
	 ...
	 * 
	 * @param gl
	 *            A GL object to reference when setting values for it
	 * @param glu
	 *            A GLU object to reference when setting values for it
	 * @param imgLoc
	 *            The string location of the image/texture to load.
	 */
	public void loadGLTextures(GL gl, GLU glu, String[] imgLoc) {
		...
		gl.glGenTextures(NUM_TEXTURES, texture, 0);
	...
	public void keyPressed(KeyEvent ke) {
		...
		default:
			if (ke.getKeyCode() < 250) // only interested in first 250 key
										// codes, are there more?
	...
	// the different types create diff. objects which are returned and tested
	// for
	// when the textures are loaded...
	// the classes themselves were created by other people whos credit is in the
	// file
	// however those files are edited by me to best suit my needs and wants for
	// my apps
	/**
	 * This program makes a series of calls to read an image and return an
	 * Object that has already been run just so that information can be obtained
	 * about the image.
	 * 
	 * @param resource
	 *            Tells the program which image to use.
	 * @return Returns an Object that is from the package LoadImageTypes
	 */
	public Object readImage(String resource) {
		if (resource.endsWith("png")) {
			...
			loadBMPImage.generateTextureInfo(getResourceB(resource), resource,
					false);
			return loadBMPImage;
		} else {
			return null;
	...
	// Definitely gotten from Kevin Duling (jattier@hotmail.com) in his ports to
	// the NeHe
	// tuts. Same with the few methods that follow. I had to change the method
	// getResource() to work
	// better for locating the image and returning a valid resource. I have also
	// changed some other
	// things but just to make it nicer and so that it fits better with my
	// scheme of programming.
	private void makeTexture(GL gl, GLU glu, Object img, int target,
			boolean mipmapped) {
		// if (mipmapped)
		// {
	...
	/**
	 * Retrieve an InputStream resource from the jar. If the resource is not
	 * found, then the local disk is also checked.
	 * 
	 * @param filename
	 *            Complete filename, including parent path
