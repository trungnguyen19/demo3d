...
package nehe.lesson17.OptionFrame;
...
public class Options extends JFrame {
	...
	public Options() {
		...
		// ImageIcon(getClass().getResource("resources/images/splash.jpg")));
		// new panels
		...
		// these are push buttons because i called the other things buttons and
		// i'm too lazy to fix them...that or too tired...:)
		JButton jButtonOk = new JButton("OK");
		JButton jButtonCancel = new JButton("Cancel");

		// create a name for all the buttons we created so we can do certain
		// things if the names match on certain actions
		...
		// add the bitsperpixel buttons to the correct panel...this currently
		// isn't used but i have been told that you can set this
		// setting in GL so it's here
		splashButtonPanel2.add(jRadioButton16bpp);
		splashButtonPanel2.add(jRadioButton32bpp);

		// add the size buttons to the correct group to specify which one is
		// true and which of the others aren't
		...
		ActionListener actionListener = new ActionListener() {
			...
			public void actionPerformed(ActionEvent ae) {
				...
				// i'm not going through everything, it's self-explainatory if
				// you follow the code...
				// check the instanceof the object
				if (actionSource instanceof JCheckBox) {
					// see if the checkbox is selected and if the name
					// matches...
					// strings, because they are objects, need to be checked
					// with the object.equals(""); convention...
					if (((JCheckBox) actionSource).isSelected()
							&& (((JCheckBox) actionSource).getName())
									.equals("Fullscreen")) {
						System.out.println("Changed the value of: "
								+ ((JCheckBox) actionSource).getName());
						fullscreen = !fullscreen;
					} else if ((((JCheckBox) actionSource).getName())
							.equals("Fullscreen")) {
						System.out.println("Changed the value of: "
								+ ((JCheckBox) actionSource).getName());
						fullscreen = !fullscreen; // this just makes it's value
													// the opposite, much easier
													// then figuring out if it
													// is checked
													// before or after you click
													// on it...just a hassle to
													// check that stuff...same
													// thing above!
					}
				} else if (actionSource instanceof JRadioButton) {
					if (((JRadioButton) actionSource).isSelected()) {
						// just setting variables...very procedural...but very
						// simple
						if ((((JRadioButton) actionSource).getName())
								.equals("640x480")) {
							System.out.println("Selected: "
									+ ((JRadioButton) actionSource).getName());
							pixels = "640x480";
							width = 640;
							height = 480;
						} else if ((((JRadioButton) actionSource).getName())
								.equals("800x600")) {
							System.out.println("Selected: "
									+ ((JRadioButton) actionSource).getName());
							pixels = "800x600";
					...
					if (((JButton) actionSource).getName().equals("OK"))
						ok = true; // this is used in the RunGL class where it
									// constantly checks to see if OK or cancel
									// were clicked...
					else if (((JButton) actionSource).getName()
							.equals("Cancel"))
						cancel = true;
				}
			}
		...
		// this next line sets the splash location...calculate the middle of the
		// screen...you can test the math...it works...
		setLocation(
				((Toolkit.getDefaultToolkit().getScreenSize().width / 2) - (getWidth() / 2)),
				((Toolkit.getDefaultToolkit().getScreenSize().height / 2) - (getHeight() / 2)));
		// splashLabel.setBorder(BorderFactory.createLineBorder(new
		// Color(75,75,75))); //generate a line around the label holding the
		// image
