...
public class JOGL2Nehe10World3D implements GLEventListener, KeyListener {
			...
			for (int i = 0; i < sector.triangles.length; i++) {
				for (int vert = 0; vert < 3; vert++) {
					...
					if (line != null) {
						...
						sector.triangles[i].vertices[vert].x = scanner
								.nextFloat();
						sector.triangles[i].vertices[vert].y = scanner
								.nextFloat();
						sector.triangles[i].vertices[vert].z = scanner
								.nextFloat();
						sector.triangles[i].vertices[vert].u = scanner
								.nextFloat();
	...
	public void display(GLAutoDrawable drawable) {
		...
		for (int i = 0; i < sector.triangles.length; i++) {
			...
			v = sector.triangles[i].vertices[0].v * textureHeight
					- textureBottom;
			gl.glTexCoord2f(u, v);
			...
			v = sector.triangles[i].vertices[1].v * textureHeight
					- textureBottom;
			gl.glTexCoord2f(u, v);
			...
			v = sector.triangles[i].vertices[2].v * textureHeight
					- textureBottom;
			gl.glTexCoord2f(u, v);
	...
	public void keyPressed(KeyEvent e) {
		...
		case VK_UP:
			...
			walkBiasAngle = (walkBiasAngle >= 359.0f) ? 0.0f
					: walkBiasAngle + 10.0f;
		...
		case VK_DOWN:
			...
			posX += (float) Math.sin(Math.toRadians(headingY)) * moveIncrement;
			posZ += (float) Math.cos(Math.toRadians(headingY)) * moveIncrement;
			walkBiasAngle = (walkBiasAngle <= 1.0f) ? 359.0f
					: walkBiasAngle - 10.0f;
			walkBias = (float) Math.sin(Math.toRadians(walkBiasAngle)) / 20.0f;
			break;
