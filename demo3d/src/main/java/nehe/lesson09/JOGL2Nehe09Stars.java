 ...
 * 
 * 't': toggle twinkle on/off 'f': switch to the next texture filters (nearest,
 * linear, mipmap) Page-up/Page-down: zoom in/out decrease/increase z
...
public class JOGL2Nehe09Stars implements GLEventListener, KeyListener {
	...
	private float textureCoordTop, textureCoordBottom, textureCoordLeft,
			textureCoordRight;
	...
	public void display(GLAutoDrawable drawable) {
		...
		for (int i = 0; i < stars.length; i++) {
			...
			// Initial 90 degree tile in the x-axis, y-axis pointing out of
			// screen
			...
			// Translate about the x-axis (pointing right) to its current
			// distance
			...
			// Rotate the axes back, so that z-axis is again facing us, to
			// ensure that
			// the quad (with texture) on x-y plane is facing us
			gl.glRotatef(-stars[i].angle, 0.0f, 1.0f, 0.0f);
			gl.glRotatef(-tilt, 1.0f, 0.0f, 0.0f);

			// Take note that without the two rotations and undo, there is only
			// one
			// translation along the x-axis
			// Matrix operation is non-commutative. That is, AB != BA.
			// Hence, ABCB'A' != C

			// Draw the star, which spins on the z axis (pointing out of the
			// screen)
			...
			if (twinkleOn) {
				...
				gl.glColor4ub(stars[(numStars - i) - 1].r,
			...
			// The star spins about the z-axis (pointing out of the screen), and
			// spiral
			// inwards and collapse towards the center, by increasing the angle
			// on x-y
			// plane and reducing the distance.

			starSpinAngle += 0.01f; // used to spin the stars about the z-axis
			// spiral pattern
			stars[i].angle += (float) i / numStars; // changes the angle of a
													// star
			// collapsing the star to the center
			stars[i].distance -= 0.01f; // changes the distance of a star
			// re-bone at the edge
			if (stars[i].distance < 0.0f) { // Is the star collapsed to the
											// center?
