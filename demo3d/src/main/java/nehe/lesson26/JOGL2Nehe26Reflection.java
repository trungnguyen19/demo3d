...
public class JOGL2Nehe26Reflection implements GLEventListener, KeyListener {
	...
	private float[] lightAmbientValue = { 0.7f, 0.7f, 0.7f, 1.0f }; // Ambient
																	// light
																	// value in
																	// RGBA
	private float[] lightDiffuseValue = { 1.0f, 1.0f, 1.0f, 1.0f }; // Diffuse
																	// light
																	// value in
																	// RGBA
	private float[] lightDiffusePosition = { 4.0f, 4.0f, 6.0f, 1.0f }; // Position
																		// of
																		// the
																		// light
																		// XYZ
	...
	public void init(GLAutoDrawable drawable) {
		...
		try {
			for (int i = 0; i < textures.length; i++) {
				textures[i] = TextureIO.newTexture(
						this.getClass().getResource(textureFileNames[i]),
		...
		// Set up lighting
		gl.glLightfv(GL_LIGHT0, GL_AMBIENT, lightAmbientValue, 0); // Set the
																	// ambient
																	// lighting
																	// for
																	// LIGHT0
		gl.glLightfv(GL_LIGHT0, GL_DIFFUSE, lightDiffuseValue, 0); // Set the
																	// diffuse
																	// lighting
																	// for
																	// LIGHT0
		gl.glLightfv(GL_LIGHT0, GL_POSITION, lightDiffusePosition, 0); // Set
																		// the
																		// position
																		// for
																		// LIGHT0
	...
	public void display(GLAutoDrawable drawable) {
		...
		gl.glEnable(GL_STENCIL_TEST); // Enable stencil buffer for "marking" the
										// floor
		gl.glStencilFunc(GL_ALWAYS, 1, 1); // Always passes, 1 Bit Plane, 1 As
											// Mask
		gl.glStencilOp(GL_KEEP, GL_KEEP, GL_REPLACE); // Set the stencil buffer
														// to 1 where we draw
														// any polygon
		...
		gl.glColorMask(true, true, true, true); // Set color mask to let RGBA
												// thru
		gl.glStencilFunc(GL_EQUAL, 1, 1); // We draw only where the stencil is 1
		// (i.e. where the floor was drawn)
		gl.glStencilOp(GL_KEEP, GL_KEEP, GL_KEEP); // Don't change the stencil
													// buffer

		gl.glEnable(GL_CLIP_PLANE0); // Enable clip plane for removing artifacts
										// (when the object crosses the floor)
		gl.glClipPlane(GL_CLIP_PLANE0, eqr, 0); // Equation for reflected
												// objects
		gl.glPushMatrix(); // Push the matrix onto the stack
		gl.glScalef(1.0f, -1.0f, 1.0f); // Mirror y-axis

		gl.glLightfv(GL_LIGHT0, GL_POSITION, lightDiffusePosition, 0); // Set up
																		// LIGHT0
		...
		gl.glDisable(GL_CLIP_PLANE0); // Disable clip plane for drawing the
										// floor
		gl.glDisable(GL_STENCIL_TEST); // We don't need the stencil buffer any
										// more

		gl.glLightfv(GL_LIGHT0, GL_POSITION, lightDiffusePosition, 0); // Set up
																		// LIGHT0
																		// position
		gl.glEnable(GL_BLEND); // Enable blending (otherwise the reflected
								// object won't show)
		...
		gl.glColor4f(1.0f, 1.0f, 1.0f, 0.8f); // Set color to white with 80%
												// alpha
		gl.glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA); // Blending based
																// on source
																// alpha and 1
																// minus dest
																// alpha
	...
	private void drawBall(GL gl) {
		...
		// to 40%
		// and enable blending based on the source alpha value. Enables sphere
		// mapping.
		// The final result is a reflection that almost looks like bright points
		// of light mapped to the beach ball.
		// Because we enable sphere mapping, the texture is always facing the
		// viewer,
		...
		gl.glBlendFunc(GL_SRC_ALPHA, GL_ONE); // Set blending mode to mix based
												// on source alpha
		gl.glEnable(GL_TEXTURE_GEN_S); // Enable sphere mapping on s-axis
		gl.glEnable(GL_TEXTURE_GEN_T); // Enable sphere mapping on t-axis
		glu.gluSphere(quadric, 0.35f, 32, 16); // Draw another sphere using new
												// texture
	...
	private void drawFloor(GL gl) {
		...
		gl.glTexCoord2f(textureRights[0], textureTops[0]); // Top-right of
															// texture
		gl.glVertex3f(2.0f, 0.0f, -2.0f); // Top-right corner of the floor
		gl.glTexCoord2f(textureRights[0], textureBottoms[0]); // Bottom-right of
																// texture
