...
public class Lesson19 {
	...
	/**
	 * Main method which prompts the user for screen mode (full? windowed?) and
	 * creates an instance of the OpenGL window.
	 */
	...
	public Lesson19() {
		...
		createGLWindow("JOGL Port for NeHe OpenGL Lession 19", WIN_WIDTH,
				WIN_HEIGHT, m_nBits);
	...
	/**
	 * Brings up a JOptionPane dialog to prompt user for preferred window mode
	...
	private void promptScreenMode() {
		...
		int option = JOptionPane.showOptionDialog(null, message,
				"Full Screen Prompt", JOptionPane.YES_NO_CANCEL_OPTION,
	...
	/**
	 * Initializes the main window frame with JOGL components which are able to
	 * display OpenGL objects.
	 * 
	 * @param width
	 *            The width of the window.
	 * @param height
	 *            The height of hte window.
	 * @param bit
	 *            The number of bits to use for color.
	 * 
	 */
	private void createGLEnabledFrame(int width, int height, int bit) {
		...
		m_oCanvas = new GLCanvas();// GLDrawableFactory.getFactory().createGLCanvas(capabilities);
	...
	protected void exitProgram() {
		...
		/*
		 * if (m_bIsFullScreen) { toggleWindowMode(); }
		 * 
		 * if (m_oMainWindow != null) { m_oMainWindow.dispose(); }
		 */
	...
	/**
	 * Toggles the application window between full screen mode and windowed
	 * mode. If the application is in full screen mode, then change it to
	 * windowed mode. If the app is in windowed mode, then change it to full
	 * screen mode.
	 */
	protected void toggleWindowMode() {
		...
		try {
			...
			showError(eHeadless, "Cannot switch between windowed and full "
	 ...
	 * 
	 * @param defaultScreen
	 *            The default screen where the window resides.
	 */
	...
	private void setFullScreenMode(GraphicsDevice defaultScreen) {
		if (defaultScreen.isFullScreenSupported()) {
			...
			try {
				...
				// show error.
				System.out
						.println("Error while attempting to switch to full screen"
								+ " mode: \n" + e.getMessage());
			} finally {
				// defaultScreen.setFullScreenWindow(null);
			}
			...
			System.out
					.println("Full screen mode is not supported on this device.");
		}
	}

	/**
	 * Turns on/off the decorated property of the main window by first making
	 * the window undisplayable, then set the decorated property.
	 * 
	 * NOTES: After this method has been executed successfully, the window will
	 * be in the undisplayable state. It is the responsibility of the calling
	 * method to enable the displayability of the window again.
	 * 
	 * @param decorated
	 *            A boolean indicating if the window should be decorated (i.e.
	 *            show title bar, etc.)
	 */
	private void setWindowUndecorated(boolean decorated) {
		...
		if (m_oMainWindow.isDisplayable()) {
			System.out
					.println("In setWindowUndecorated(), window is displayable. Disposing "
		...
		try {
			...
			System.out
					.println("Error while setting window decorated property. "
							+ e.getMessage());
		}
	}
	 ...
	 * 
	 * @param error
	 *            The error generated. Can be null. If error is null, then
	 *            displays the string represented by the param 'errorMessage'.
	 * @param errorMessage
	 *            Any special error messages to display. If errorMessage is
	 *            null, then display the message associated with the exception.
	 */
	protected void showError(Exception error, String errorMessage) {
		...
		JOptionPane.showMessageDialog(m_oMainWindow, displayMessage, "Error",
				JOptionPane.ERROR_MESSAGE);
		// }
	}

	/**
	 * Toggles the currently displayed texture. The counter m_nCurrentFilter is
	 * used to select which texture filter to use. We will create 3 texture
	 * mappings later on in the application. Therefore, the counter will range
	 * from 0 to 2, inclusive. Each time this method is called, the counter will
	 * increase by 1.
	 */
	 ...
	 * 
	 * @param increase
	 *            A boolean indicating if the downward pull on the particles
	 *            should increase. If true, the downward pull (negative Y)
	 *            increases. If false, the downward pull deceases.
	 */
	 ...
	 *            A boolean indicating if the downward pull on the particles
	 *            should increase. If true, the downward pull (negative Y)
	 *            increases. If false, the downward pull deceases (or upward
	 *            pull increases).
	 */
	...
	/**
	 * Speeds up or slows down the particles by decreasing or increaseing the
	 * slow down factor.
	 *
	 * @param increase
	 *            A boolean indicating if the particles should travel faster or
	 *            slower. If true, the slow-down factor increases, therefore the
	 *            particles travel slower. If false, the slow-down factor
	 *            decreases, therefore the particles travel faster.
	 */
	 ...
	 * 
	 * @param increase
	 *            A boolean indication whether to increase or decrease the speed
	 *            in the x direction. If true, the particles' speed in the
	 *            positive x direction increases. If false, the particles' speed
	 *            in the negative x direction increases.
	 * 
	 */
	 ...
	 * @param increase
	 *            A boolean indication whether to increase or decrease the speed
	 *            in the y direction. If true, the particles' speed in the
	 *            positive y direction increases. If false, the particles' speed
	 *            in the negative y direction increases.
	 * 
	 */
