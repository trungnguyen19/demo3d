...
public class GLEventHandler implements GLEventListener {
	 ...
	 * 
	 * @param drawable
	 *            The object capablable of drawing OpenGL objects.
	 */
	public void init(GLAutoDrawable drawable) {
		...
		loadBMP("src/main/java/nehe/lesson19/data/Particle.jpg");
		...
		resizeGLScene(drawable, drawable.getWidth(), drawable.getHeight());
	 ...
	 * 
	 * @param drawable
	 *            The OpenGL drawable used to display the particles.
	 */
	 ...
	 */
	private void resizeGLScene(GLAutoDrawable drawable, double width,
			double height) {
	 ...
	 *
	 * @param drawable
	 *            The OpenGL drawable used to display objects.
	 */
	private void loadGLTextures(GLAutoDrawable drawable) {
		GL gl = drawable.getGL();
		// Set up OpenGL to generate 1 texture.
		gl.glGenTextures(1, m_aTextures, 0);
	 ...
	 *            The texture ID.
	 * @param filterTypeMin
	 *            The filter type to use for image is smaller than the object.
	 * @param filterTypeMax
	 *            The filter type to use when image is larger than the object.
	 */
	private void generateTexture(GL gl, int id, int filterTypeMin,
			int filterTypeMax) {
		// Bind the texture to 2D
		gl.glBindTexture(GL.GL_TEXTURE_2D, m_aTextures[id]);

		ByteBuffer unpackedPixels = ByteBuffer
				.allocate(m_aImagePixelBytes.length);
		unpackedPixels.put(m_aImagePixelBytes);
		unpackedPixels.rewind();
		// Create the actual texture passing in the pixel byte array.
		gl.glTexImage2D(GL.GL_TEXTURE_2D, 0, 3, m_oBufferedImage.getWidth(),
				m_oBufferedImage.getHeight(), 0, GL.GL_RGB,
	 ...
	 * @param particle
	 *            The Particle object to display.
	 */
	...
	private void updateParticle(Particle particle) {
		...
		particle.setXLocation(particle.getXLocation() + particle.getXSpeed()
				/ (m_oParent.m_fSlowDown * 1000));
		particle.setYLocation(particle.getYLocation() + particle.getYSpeed()
				/ (m_oParent.m_fSlowDown * 1000));
		particle.setZLocation(particle.getZLocation() + particle.getZSpeed()
				/ (m_oParent.m_fSlowDown * 1000));
		...
		particle.setXGravity(particle.getXGravity()
				+ m_oParent.m_fXGravityChange);
		particle.setYGravity(particle.getYGravity()
				+ m_oParent.m_fYGravityChange);
	}

	/**
	 * Handles the dead particle given. A dead particle is restarted by given it
	 * full life, new color, new speeds, etc.
	 * 
	 * @param gl
	 *            The OpenGL reference.
	 * @param particle
	 *            The dead particle to be processed.
	 */
