...
public class JOGL2Nehe11Flag implements GLEventListener { // Renderer
	...
	private static float rotateAnleX = 0; // rotational angle for x-axis in
											// degree
	private static float rotateAnleY = 0; // rotational angle for y-axis in
											// degree
	private static float rotateAngleZ = 0; // rotational angle for z-axis in
											// degree
	...
	public void init(GLAutoDrawable drawable) {
		...
		try {
			...
			texture = TextureIO.newTexture(
					this.getClass().getResource(textureFileName), false,
					textureFileType);
		} catch (GLException e) {
			e.printStackTrace();
		} catch (IOException e) {
		...
		for (int x = 0; x < numPoints; x++) { // Loop Through The Y Plane
			for (int y = 0; y < numPoints; y++) {
				...
				// xmax is 45. Get 9 after dividing by 5. Subtract 4.5 to
				// centralize.
				points[x][y][0] = (float) x / 5.0f - 4.5f;
				points[x][y][1] = (float) y / 5.0f - 4.5f;
				// Sine wave pattern
				points[x][y][2] = (float) (Math.sin(Math
						.toRadians(x / 5.0f * 40.0f)));
			}
		}
	}
	...
	public void display(GLAutoDrawable drawable) {
		...
		if (wiggleCount == 2) { // Used To Slow Down The Wave (Every 2nd Frame
								// Only)
			for (int y = 0; y < 45; y++) {
				float tempHold = points[0][y][2]; // Store current value One
													// Left Side Of
				// Wave
				for (int x = 0; x < 44; x++) {
				...
				points[44][y][2] = tempHold; // Last Value Becomes The Far Left
												// Stored
				// Value
			}
